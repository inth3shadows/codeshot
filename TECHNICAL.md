...
runCodegraph(['callees', symbol, '--path', repoPath, '--limit', limit, '--json'])
  --- parseCodegraphOutput detects codegraph's plain-text "Symbol not found"
      response (via matchSymbolNotFound) and exits with a clean error instead
      of a raw JSON-parse-failure message
        |
        v
...
```bash
npm test              # runs test/run.js — assertions against all exported pure functions, plus two CLI-level tests (depth 1, and --depth 2) against this repo's own codegraph index (skips if not codegraph-indexed)
...
- **The rendered root-node label can be wrong for a fuzzy/partial query, and this is a deliberate, documented punt, not an oversight.** `codegraph callers`/`callees --json` fuzzy-matches an inexact symbol query (e.g. querying `New` resolves to `NewOracle` if no literal `New` exists) but its JSON response's top-level `"symbol"` field just echoes the raw query string back — there is no field anywhere in that contract indicating the resolved canonical name. The *data* codeshot draws is correct (real callers/callees of whatever codegraph actually resolved to); only the root box's *label* is wrong, showing the literal query instead of the resolved name. A clean fix exists — `codegraph query <name> --json --limit 1` (a separate, real, documented JSON contract) returns the resolved canonical `name` — but adding that as a mandatory preflight call would cost every single invocation an extra sequential `codegraph` subprocess spawn (~0.4-0.5s observed, comparable to the existing two-call default-depth cost) to fix a cosmetic issue that only manifests on non-exact queries. Investigated and deliberately not implemented for that reason; revisit if fuzzy-querying turns out to be common enough in practice to justify the tax on every exact-match invocation too.
